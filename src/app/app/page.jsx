...
export default function AppPage() {
    ...
    async function loadMatches(isSilent = false) {
        ...
        try {
            ...
            // V50.8: DELAYED ORACLE - Wait 1.2s and ensure user is still here
            setTimeout(async () => {
                if (!user) return;
                console.log('🔮 [Oracle] Iniciando análisis profundo (Network Shield Active)...');
                ...
                for (let i = 0; i < priorityMatches.length; i += chunkSize) {
                    if (!user) break; // V50.8: Stop if logged out
