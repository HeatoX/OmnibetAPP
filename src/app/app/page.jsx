...
export default function AppPage() {
    ...
    async function loadMatches(isSilent = false) {
        ...
        try {
            ...
            // V50.10 PHASE 3: SECURE SEQUENTIAL ORACLE
            setTimeout(async () => {
                if (!user || document.visibilityState !== 'visible') return;

                console.log('🔮 [Seq] Fase 3: Activando Oráculo (Modo Secuencial V50.10)...');
                setOracleStats({ analyzed: 0, highConfidence: 0 });

                // Strictly sequential processing to avoid any network crash
                for (const match of rawMatches.slice(0, 15)) {
                    if (!user) break;

                    try {
                        console.log(`🧠 [Oráculo] Analizando: ${match.home.name} vs ${match.away.name}`);
                        const analysis = await analyzeMatchDeep(match);

                        // Update UI match by match
                        ...
                        // Yield to browser UI and wait for next network slot
                        await new Promise(r => setTimeout(r, 1200));
                    } catch (err) {
                        console.warn(`Skip match ${match.id}:`, err.message);
                    }
                }
                console.log('🏁 [Seq] Análisis del Oráculo finalizado con éxito.');
            }, 4500); // 4.5s Buffer after login
